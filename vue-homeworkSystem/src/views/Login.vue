...
export default {
  ...
  methods: {
    message (message) {
      Notification({
        title: '消息通知',
        message: '用户：user 密码：123456|管理员：admin 密码：123456',
        duration: 0
      })
    },
    ...
    async getVerifyCode () {
      const { data: res } = await this.$http.get('/api/hs/user/default')
      if (res.success) {
    ...
    regist () {
      ...
      this.$refs.registForm.validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/api/hs/user/register', param).then((res) => {
            // 关闭新增窗口
    ...
    login () {
      this.$refs.loginFormRef.validate(async valid => {
        ...
        const { data: res } = await this.$http.post('/api/login', param)
        if (res.success !== true) {
          this.$message.error(res.message)
          ...
          // 2. 通过编程式导航跳转到后台主页
          if (res.data.user.currentUserInfo.role === 'student' || res.data.user.currentUserInfo.role === 'teacher') {

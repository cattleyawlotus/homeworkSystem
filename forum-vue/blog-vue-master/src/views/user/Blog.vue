  ...
  methods: {
    ...
    async deleteComment (item) {
      this.$confirm('若该评论有子评论的话会被一起删除，你确定要继续删除吗？', '提示', { // 确认框
        ...
        this.$http.delete(`/api/forum/comment/deletecomment?cid=${item.id}`).then((res) => {
          if (res.data.success) {
    ...
    async sendComment () {
      if (!this.replyComment) {
        ...
        const blogId = sessionStorage.getItem('blogId')
        if (this.toLogin()) {
          await this.$http.post(`/api/forum/comment/comment?bid=${blogId}&content=${this.replyComment}`)
        }
        this.replyComment = ''
        input.innerHTML = ''
        this.getCommentList()
    ...
    async sendCommentReply (i, pcid) {
      if (!this.replyComment) {
        ...
        const blogId = sessionStorage.getItem('blogId')
        if (this.toLogin()) {
          await this.$http.post(`/api/forum/comment/comment?bid=${blogId}&content=${this.replyComment}&pcid=${pcid}`)
        }
        this.replyComment = ''
